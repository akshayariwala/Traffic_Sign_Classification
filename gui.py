...
def classify(file_path):
    """Classifies the selected image using the trained model."""
    global label
    ...
    pred = np.argmax(model.predict(image), axis=-1)[0]  # Get predicted class index
    sign = classes.get(pred + 1, "Unknown")  # Get the corresponding label
    
    print("Predicted Sign:", sign)
    label.configure(foreground='#011638', text=sign)

def show_classify_button(file_path):
    """Displays the 'Classify Image' button after an image is uploaded."""
    classify_b = Button(top, text="Classify Image", command=lambda: classify(file_path), padx=10, pady=5)
    classify_b.configure(background='#364156', foreground='white', font=('arial', 10, 'bold'))
    classify_b.place(relx=0.79, rely=0.46)

def upload_image():
    """Handles image selection and displays it in the GUI."""
    try:
